...
bool startMaking(int masterID, int furnitureID) {
	printf("------------------- [%d] startMaking - %d, %d (%d), %d\n", currentTime, masterID, furnitureID, furniture[furnitureID].makingTime, master[masterID].startMakingTime);
...
bool completeMaking(int masterID) {
	printf("------------------- [%d] CompleteMaking - %d, %d, %d\n", currentTime, master[masterID].furnitureID, furniture[master[masterID].furnitureID].makingTime, currentTime - master[masterID].startMakingTime);
	int furnitureID;
	if (masterID <= 0 || masterID > masterCount ||
...
int main() {
	...
	for (testcase = 1; testcase <= TC; ++testcase) {
		...
		if (success && earned * 100 / total >= PRODUCTION_RATE)
			...
			totalscore -= 10000000;
		printf("#%d %d / %d / %d\n", testcase, earned, total, totalscore);
	}
